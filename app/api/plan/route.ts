  ...
  try {
    ...
    const res = await anthropic.messages.parse({
      ...
      messages: [
        {
          role: "user",
          content: `아래 사람에게 맞는 1주일 운동 계획을 세워주세요.
...
- 현실적인 목표 도달 기간: 약 ${daily.realisticWeeks}주`,
    ...
    if (!res.parsed_output) {
      return NextResponse.json({ error: "계획을 만들지 못했습니다. 다시 시도해주세요." }, { status: 502 });
    }
    return NextResponse.json({ daily, plan: res.parsed_output });
  } catch (e) {
